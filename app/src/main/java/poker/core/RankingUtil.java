...
import static poker.core.CardRank.*;
import static poker.core.Ranking.*;
...
public class RankingUtil {
	...
	public static List<Card> getRoyalFlush(IPlayer player, List<Card> tableCards) {
		...
		List<CardRank> rankEnumList = toRankEnumList(player, tableCards);
	...
	//S 9o 5 cartas seguidas de naipes diferentes, caso empate ganha aquele com a maior sequência.
	...
	private static Boolean isSameSuit(IPlayer player, List<Card> tableCards) {
		CardSuit suit = player.getCards()[0].getSuit();
	...
	private static List<CardRank> toRankEnumList(IPlayer player,
			List<Card> tableCards) {
	...
	private static void setRankingEnumAndList(IPlayer player,
			Ranking rankingEnum, List<Card> rankingList) {
